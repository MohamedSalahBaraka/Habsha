...
import { DishOrder, Dish as DishType, OrderProduct } from "@/types/Models.t";
...
export default function OrderDetailsStore() {
  ...
  const { id = "2p" } = params;
  const [dishes, setdishes] = useState<OrderProduct[]>();
  const [isFetchDataError, setIsFetchDataError] = useState(false);
  const fetchData = useCallback(async () => {
    try {
      setIsFetchDataError(false);
      const data = await StoreService.getorder(parseInt(id.toString()), config);
      setdishes(data.order.store_order_product);
  ...
  const renderdishes = dishes?.map((item, index) => {
    return (
      <Product
        Colors={Colors}
        key={index}
        uri={ImageUrl + item.product?.photo}
