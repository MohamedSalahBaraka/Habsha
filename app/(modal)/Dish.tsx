...
import { Dish, Size } from "@/types/Models.t";
...
export default function DishScreen() {
  ...
  const addToCart = useCallback(async () => {
    try {
      ...
      if (typeof size == "undefined") {
        Alert.alert("خطأ", "عليك اختيار حجم");
        return;
      }
      setIsLoading(true);
      await CartService.add({ count, dish_id: parseInt(id.toString()), size_id: size }, config);
      router.back();
    } catch (error) {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setIsLoading(true);
      const data = await GetService.getDish(parseInt(id.toString()), config);
      setdish(data.dish);
      setsizes(data.dish.sizes);
      setIsLoaded(true);
    } catch (error) {
