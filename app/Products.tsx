...
import { Category, Dish, Product } from "@/types/Models.t";
...
export default function Products() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const data = await StoreService.getproducts(config, bodyParameters);
  ...
  const renderproducts = products?.map((cb, index) => {
    return <DishComponent Colors={Colors} id={cb.id} key={index} Title={cb.name} uri={cb.photo} price={cb.options[0].price} />;
  });
  const networkback = () => {
        ...
        <>
          <ScrollView>
            ...
            >
              ...
              <Text style={[barakocta.tRight, barakocta.my3, { color: Colors.tertiary, fontFamily: "Cairo", fontSize: 20 }]}>المنتجات</Text>
              <View style={[barakocta.fRow, barakocta.fWrap]}>{renderproducts}</View>
            </View>
          </ScrollView>
          <TouchableOpacity
