...
export default function Restaurant() {
  ...
  const renderrestaurante = restaurants?.map((cb, index) => {
    return <Resturant Colors={Colors} user_id={cb.id} Title={cb.name} uri={cb.photo} key={index} />;
  });
  const fetchData = useCallback(async () => {
    try {
      setIsFetchDataError(false);
      const data = await GetService.getRestourante(config);
  ...
  const search = useCallback(async () => {
    try {
      ...
      const data = await GetService.getRestourante(config, keyword);
      setrestaurants(data.restaurants);
      setIsLoaded(true);
      setIsLoading(false);
    } catch (error) {
        ...
        <>
          <ScrollView>
            ...
            >
              ...
              <Text style={[barakocta.tRight, barakocta.my3, { color: Colors.tertiary, fontFamily: "Cairo", fontSize: 20 }]}>المطاعم</Text>
