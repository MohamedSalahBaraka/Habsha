...
export default function CheckOutStore() {
  ...
  const sendForm = useCallback(async () => {
    try {
      ...
      setIsLoading(true);
      setIsError(false);
      await StoreService.placeOrder({ city, neighbourhood, details, name, phone }, config);
      router.push("(drawer)/StoreFormerOrders");
      setIsLoading(false);
    } catch (error) {
      setIsError(true);
  ...
  const GetDelivaryPrice = useCallback(async (city: number) => {
    try {
      const data = await StoreService.orderDelivaryPrice(city, config);
